...
function Table({
    ...
    const resultData = data.filter(res => res.title === 'CIPC Enterprise Listing');
    ...
    useEffect(() => {
        ...
        colDefs.length = 0;
        if(resultData[0].search[0].CIPCEnterprises[0]){
            const keys = Object.keys(resultData[0].search[0].CIPCEnterprises[0]);
            keys.forEach(key => columnDefs.push({field: key}))
            setColumnDefs(colDefs)
            setRowData(() => resultData.map((res) => {
                const filter = res.search[0].CIPCEnterprises.map((jug) => {
                    return jug
                })
  ...
  return (
    <div className="h-116 w-full">
        <input
        type="text"
        className="mb-4 p-1 w-110 rounded bg-white border border-neutral-200"
