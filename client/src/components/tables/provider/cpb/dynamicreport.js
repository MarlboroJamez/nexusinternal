...
function Personlist({data}) {
  ...
  const downloadPDF = () => {
    const url = `data:application/pdf;base64,${data[0].search[0].Results[0].EncodedPDF}`;
        ...
        <div id="pdf-div" className='w-full p-6'>
          ...
          <div>
            {cookieValue && cookieValue.isActivity ? (
              <iframe
              title="PDF Viewer"
              src={`data:application/pdf;base64,${data[0].search[0].Results[0].EncodedPDF}#toolbar=0&navpanes=0&scrollbar=0&view=FitH`}
