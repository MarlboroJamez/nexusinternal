...
function Table({ data }) {
  ...
  const headers = data.length > 0 ? Object.keys(data[0]).filter(item => item !== "search" && item !== "__v" && item !== "license" && item !== "package" && item !== "resetPasswordExpire" && item !== "resetPasswordToken" && item !== "role" && item !== "accessibility" && item !== "isAdmin" && item !== "isExport" && item !== "isReporting" && item !== "isDirectory"  && item !== "isAnalysis" && item !== "isActivity" && item !== "isScheduled" && item !== "isRegular"  && item !== "_id"):"";
        ...
        <tr key={item.Id} className="h-fit border border-neutral-200 p-2 text-xs">
          {headers.map((header) => {
            ...
            if (header === "cost") {
              ...
              cellValue = new Date(item[header]).toLocaleDateString("en-US", dateOptions);
            } else if (typeof cellValue === "boolean") {
              cellValue = cellValue ? "true" : "false";
            }
  
            return (
              <td key={`${header}-${item.Id}`} className={cellClass}>
                {cellValue}
              </td>
            );
          })}
