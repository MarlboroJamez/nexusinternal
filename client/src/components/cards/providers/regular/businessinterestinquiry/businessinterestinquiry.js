    ...
    const handleSubmission = async (e) => {
        ...
        try {
                if(reference.length === 0){
                    ...
                    const {data} = await axios.post('/api/v1/provider/cpb/search/businessinterestinquiry', {
                ...
                <div className="grid ml-10 h-fit">
                    <div className='mb-6 grid'>
                        <div className='flex w-fit mt-5'>
                            ...
                            <input 
                                value={useDHAExtra}
                                onChange={(e) => setUseDHAExtra(e.target.checked ? "Yes" : "")}
                                type="checkbox"
                                placeHolder="UseDHAExtra"
                                className="mt-1 h-fit w-fit mr-3"
                            />
                            <label 
                            className='text-sm w-full text-right font-bold mb-0.5'>Use DHA Extra</label>
                        </div>
                    ...
                    <div className='mb-6 grid'>
                        ...
                        <input 
                        value={useSAFPDirect}
                        onChange={(e) => setUseSAFPDirect(e.target.checked ? "Yes":"")}
                        type="checkbox"
                        placeHolder="UseSAFPExtra"
                        className="mt-1 h-fit w-fit mr-3"/>
                        <label className='text-sm w-full text-right font-bold mb-0.5'>Use SAFP Extra</label>
                        </div>
                        <label className='mt-1 w-80 text-xs'>Only use if you properly understand</label>
                    </div>
                        <button 
