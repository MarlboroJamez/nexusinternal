...
// Assets
import Guide from '../../../../../assets/guides/lifestyle.pdf'
...
// Models
import HelperModel from '../../../providers/helpers/models/helperlifestylereport';
...
function Personlisting({selectedProject, share, notified, project}) {
    ...
    const [inputAddress, setInputAddress] = useState(JSON.stringify({},null,2))
    const [inputPerson, setInputPerson] = useState(JSON.stringify({}, null, 2));
        ...
        const licenseInfo = async () => {
            if(cookieValue){
                if(cookieValue.license){
                    ...
                    setLicense(data.data[0].restrictions.filter((res) => res.type === "Lifestyle Audit Report"))
                    setAccess(cookieValue.license[0].restrictions.filter(item => item.type === "Lifestyle Audit Report"))
                }
            }
        }
        licenseInfo()
    ...
    const handleSubmission = async (e) => {
        e.preventDefault();
        const lic = cookieValue.license[0].restrictions.filter((res) => res.type === "Lifestyle Audit Report")
        const cost = lic[0].cost
        const currentTotal = cookieValue.license[0].totalIncome;
        
        try {
                if(reference.length === 0 || idNumber === 0){
                    setError(true)
                } else {
                    if(license[0].searches > 0){
                        setLoading(true)
                    setRunningModel(true)
                    const {data} = await axios.post('/api/v1/provider/cpb/search/lifestyleauditreport', {
                        ...
                        share: true,
                        restrictionName: "Lifestyle Audit Report",
  ...
  return (
    <div>
        ...
        {view === "search" && (
            ...
            <table className='w-full text-left mt-10'>
                ...
                <tbody className='text-sm'>
                    <tr>
                        ...
                        <td className='p-2 border border-neutral-200'>Lifestyle audits </td>
            ...
            <div className="flex mt-12 w-fit">
                ...
                <div className="grid ml-10 h-fit">
                    {/* PERMISSIBLE PURPOSE */}
                    <div className='mb-6 grid'>
                        <label className='mt-5 text-xs w-full text-right font-bold mb-0.5'></label>
                        <select 
